var my_app = {
  "title":"Hello",
  "quiz":"Would you like to take a quiz about Brussels?",
  "noquiz":"Thank you for your collaboration. Have a good day.",
  "questions":{
    "q1":{
      "q":"Would you know how many inhabitants does Brussels have approximately?",
      "correct":1,
      "good":"Exactly! That is quite a few",
      "bad":"No, it is 1.2 million. Sorry.",
      "img":"img/1.jpg",
      "answers":{
        "a1":"A) 1.2 million",
        "a2":"B) 0.73 million",
        "a3":"C) 3 million",
        "a4":"D) 2.6 million"
      }
    },
    "q2":{
      "q":"Could you tell me how many communes integrate the Brussels-Capital Region?",
      "correct":3,
      "good":"You got it right. Well done.",
      "bad":"I am sorry, but that is not right. There are 19.",
      "img":"img/3.jpg",
      "answers":{
        "a1":"A) 15",
        "a2":"B) 21",
        "a3":"C) 19",
        "a4":"D) 32"
      }
    },
    "q3":{
      "q":"Which of the following European Union institutions are headquartered in Brussels?",
      "correct":4,
      "good":"Very well done, that was not easy. ",
      "bad":"No, I would say they are the European Commission, the European Parliament and the Council.",
      "img":"img/11.jpg",
      "answers":{
        "a1":"A) European Commission, Council of the EU, European Central Bank",
        "a2":"B) European Commission, European Parliament, Court Justice of the EU",
        "a3":"C) European Commission, Council of the EU, Court of Auditors",
        "a4":"D) European Commission, European Parliament, Council of the EU"
      }
    },
    "q4":{
      "q":"Could you tell me when Brussels was founded?",
      "correct":2,
      "good":"That is correct. It is a thousand years old.",
      "bad":"Wrong answer, I would say it was founded in 979.",
      "img":"img/13.jpg",
      "answers":{
        "a1":"A) 1130",
        "a2":"B) 979",
        "a3":"C) 253",
        "a4":"D) 1943"
      }
    },
    "q5":{
      "q":"Would you mind to tell me the name of a very well-known festival hold in Brussels every year?",
      "correct":2,
      "good":"Indeed, it is the Iris Festival!",
      "bad":"I am sorry but you got it wrong this time. It is the Iris Festival!",
      "img":"img/14.jpg",
      "answers":{
        "a1":"A) Pupil Festival",
        "a2":"B) Iris Festival",
        "a3":"C) Eyelids Festival",
        "a4":"D) Eyebrows Festival"
      }
    },
    "q6":{
      "q":"Would you be able to tell me how many days of rain per year are there in Brussels?",
      "correct":3,
      "good":"Exactly! I recommend you to buy an umbrella if you don't have one.",
      "bad":"That is wrong, there are approximately 200 days of rain per year.",
      "img":"img/15.jpg",
      "answers":{
        "a1":"A) 364",
        "a2":"B) 170",
        "a3":"C) 200",
        "a4":"D) 310"
      }
    },
    "q7":{
      "q":"Could you tell me which of the following is the largest group of foreign residents?",
      "correct":1,
      "good":"Exactly. They are the French",
      "bad":"Oh, I thought you would knew this one. They are the French.",
      "img":"img/16.jpg",
      "answers":{
        "a1":"A) French",
        "a2":"B) Romanian",
        "a3":"C) Italian",
        "a4":"D) Moroccan"
      }
    },
    "q8":{
      "q":"Would you know how Brussels-Capital Region is ranked by population in the European Union?",
      "correct":3,
      "good":"Well done, you are right.",
      "bad":"You are wrong, it is actually ranked as the 12th largest capital city of the EU.",
      "img":"img/18.jpg",
      "answers":{
        "a1":"A) 8th",
        "a2":"B) 5th",
        "a3":"C) 12th",
        "a4":"D) 18th"
      }
    }
  }
}
